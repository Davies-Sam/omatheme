...
actually yours": a fork still carries its parent's lock screen colors,
keyboard color and boot logo, and publish says nothing about it. It also
makes the tool itself shippable: a committed test suite, screenshots, a
dependency check, and going public.
...
- A theme dir also ships **`shell.lock.toml`** (lock screen colors, read by
  omarchy-shell's idle/lock service), **`keyboard.rgb`** (RGB keyboard
  color, consumed by `omarchy-theme-set-keyboard` — ASUS ROG / F16 hardware
  only, harmless no-op otherwise), and **`unlock.png`** (the Plymouth
  boot/unlock logo). None are regenerated by `omarchy theme set`; a fork
  keeps the parent's copies forever. This is the identity gap milestone 3
  closes.
- `keyboard.rgb` is a bare hex with no `#` (tokyo-night: `ff00ff`).
...
- In tokyo-night's `shell.lock.toml` all six color keys equal `foreground`
  — the stock mapping is lazy, so a smarter derivation is an improvement,
  not a deviation.
...
A fork's `shell.lock.toml` and `keyboard.rgb` still hold the parent's
colors after every palette edit.

- [ ] **8a. `derive` on `omatheme-palette`.** For a user-owned theme
      (refuse stock with a pointer to fork first): rewrite
      `shell.lock.toml`'s color values from the current palette — choose
      the key mapping while building (a starting point: `text` /
      `placeholder` → foreground, `text-error` / `border-error` → red,
      `border` → muted, `border-active` → accent) and write it down in a
      comment — preserving the file's layout so a diff shows only value
      changes. Write `keyboard.rgb` as accent's bare hex (no `#`). Print
      what changed; no-op with a message when a file is absent.
      **Done when:** on a fork with an edited palette, derive rewrites both
      files; a second derive is byte-identical (idempotent); the layout of
      `shell.lock.toml` survives (only values differ from the parent's
      copy); stock themes are refused.
- [ ] **8b. UI hook.** Either run derive as part of the Palette panel's
      apply flow or give it its own action near the fork row — auto-derive
      on every apply would clobber hand-tuned lock colors, so weigh that;
      decide while building and write down which.
      **Done when:** reachable from the GUI on a fork, refused/disabled for
      stock themes, and the files on disk follow a palette change made
      through the app.
...
- [ ] **10a. `check` on `omatheme-publish`,** also run automatically before
      ...
      `unlock.png`, `shell.lock.toml`, `keyboard.rgb`, `neovim.lua`,
      `vscode.json`. Warn only — editor configs stay hand-edited (see out
      of scope). Point each warning at its fix (regen previews / derive /
      logo / "edit by hand").
...
- [ ] **11a. `tests/run`.** A bash suite exercising every helper end to end
      ...
      and remove (use a local file; no network) → derive → preview regen
